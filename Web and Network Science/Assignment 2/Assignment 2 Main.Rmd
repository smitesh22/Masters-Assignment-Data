...
#-----------------------------Q1.3----------------------------------------------
#plotting the graph
plot_graph = function(association_network, 
  ...
  #creating edges
  geom_edge_link(start_cap = circle(3.5, "mm"),
                   end_cap = circle(3.5, "mm"),
                   edge_width = 0.2,
                   alpha = 0.2)+
  #parameters for nodes
  #setting node size for word pairs different than other
  geom_node_point(aes(size = ifelse(V(association_network)$name %in%
                                    c(target_word1, target_word2), 
                                    degree(association_network)*100,
                                    degree(association_network)*50)), 
                    alpha = 0.7, 
                    repel = TRUE,
  ...
  geom_node_text(
      ...
      size = degree(association_network)/10,
      repel = TRUE
...
#plotting network
plot_graph(word_association_network1_rand, "COURT", "LAWYER", 1,
           "Random Walk")
plot_graph(word_association_network1_page, "COURT", "LAWYER", 2,
           "Personalised Page Rank")
#plotting network
plot_graph(word_association_network2_rand, "CHILDREN", "PARENTS", 3,
           "Random Walk")
plot_graph(word_association_network2_page, "CHILDREN", "PARENTS", 4,
           "Personalised Page Rank")
#plotting the graph
plot_graph(word_association_network3_rand, "KING", "ROYAL", 5,
           "Random Walk")
plot_graph(word_association_network3_page, "KING", "ROYAL", 6,
           "Personalised Page Rank")
```
...
\large Section 5.1 [Q2.1, Q2.2] : Creation of community clusters and explainations of the algorithms used.
...
association networks above give the better results. Modularity of the clustering network
and context of the communities founds were used to quantify if the community  detection
results were good.
...
\large Section 5.2 [Q2.3] : Detecting a appropriate label for the communities and printing the table
...
\large Section 6: Interpretation of the communities found in word association networks
...
plot_table_interpretation(interpretation6, "Children", "Parent",
                          "Page Rank", cluster3_page)

```

\large Section 7 : Comparison of Page Rank and Random Walk along with the Conclusion 

Random walk as a process can be described as movement of a point randomly from one vertex to another based on some probability, the probability is related to the weights assigned to the edge. Whereas, in Personalised Page Rank a variant of page rank we add 'bias' to certain nodes (wordpairs in this case) which will be reflected in the chances of visiting the nodes in a network based on the magnitude of the personalised page rank value.

While comparing centralities of networks it is observed that centrailites in word association network created using Personalised Page rank are more coherent compared to random walk. The reason
might lie in the idea that personalised page rank is more bias on ranking nodes based on user input
than random walk. This  statement holds true on the communities created using different clustering algorithms as well, During the trial and error phase of community detection process it was observed that communites generated using personalised page rank where optimal relative to the communities generated using random walk.
